# verda object-storage -- S3-compatible object storage
...
| `verda object-storage sync` | Sync a directory and a prefix both directions |
...
- For incomplete **uploads** specifically, `verda object-storage ls-uploads` lists them and
  lets you pick one to resume; the staged parts cost storage until completed or
  aborted (`verda object-storage abort-uploads`).
- Interactive downloads from the `verda object-storage ls` browser (per-object **Download**
...
Steps are skipped individually when the corresponding flag is already set — so `verda object-storage configure --access-key X --endpoint Y` only prompts for the secret and region, and `--profile staging` skips the profile picker entirely (targeting `[staging]`).
