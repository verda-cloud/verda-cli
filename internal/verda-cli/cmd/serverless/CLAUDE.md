...
### Container templates (`--from`)

`container create --from <name|path>` pre-fills opts from `~/.verda/templates/container/<name>.yaml` (`template_apply.go`): precedence is flag (`Flags().Changed`) > template > built-in default, field by field. A bare `--from` shows a picker (`NoOptDefVal` + positional recombine, mirrored from `vm create`); in agent mode a bare `--from` is an error. The template wizard (`template_wizard.go`) reuses the shared step factories minus `name` plus a description step — never fork step definitions. Templates store durations as strings; `ContainerSpec.MinReplicas` is a `*int` so an explicit 0 (scale-to-zero) survives save/load. A malformed duration in a template fails at apply time naming the field; a structurally bad template fails at load naming the file (`template.Validate`).

### Compute catalog joins two endpoints

`stepCompute`'s picker is backed by `joinComputeCatalog` (`compute_catalog.go`): `/serverless-compute-resources` is the availability spine (one row per `(name, size)` — confirmed against a prod capture), `/container-types` (`apiCache.fetchContainerTypes`) only enriches. **Join key is `ContainerType.Model == ComputeResource.Name` plus the size axis** (`gpu.number_of_gpus`, or `cpu.number_of_cores` for CPU flavors whose GPU count is 0) — the endpoints share no `name` vocabulary (`RTX PRO 6000` vs `RTX PRO 6000 96GB`). Unmatched rows (A100s, RTX 6000 Ada) stay selectable with `Enriched: false` and the legacy `name  (size N)` label plus a ` · VRAM and price unavailable` suffix (the wizard's select renders labels only — `Choice.Description` never reaches the output — so the label is the only place the gap can be visible); a `/container-types` failure degrades the whole step likewise. The fetch error's debug detail is `TODO(order-006)`. `gpu_memory.size_in_gigabytes` and `serverless_price` are **per-row totals** for that (flavor, size) — display them as-is; never multiply a per-unit figure (8× RTX PRO 6000 is 16.63, not 2.08×8). Per-GPU VRAM is total ÷ GPUs, division only. The price column follows the earlier `compute-type` choice (spot vs on-demand), which is why `stepCompute` declares `DependsOn: [compute-type]` — back-navigation reloads the picker with the right prices.
