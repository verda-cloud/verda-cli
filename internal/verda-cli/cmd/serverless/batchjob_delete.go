...
func newCmdBatchjobDelete(f cmdutil.Factory, ioStreams cmdutil.IOStreams) *cobra.Command {
	var yes bool
	var timeoutMs int
	...
	cmd := &cobra.Command{
		...
		Short:   "Delete a batch-job deployment",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runBatchjobDelete(cmd, f, ioStreams, name, yes, timeoutMs)
		},
	}
...
func runBatchjobDelete(cmd *cobra.Command, f cmdutil.Factory, ioStreams cmdutil.IOStreams, name string, yes bool, timeoutMs int) error {
	...
	if !yes {
		confirmed, err := confirmDestructive(
			...
			fmt.Sprintf("Deployment %q will stop accepting jobs immediately.", name),
