...
// Agent mode must refuse delete without --yes (before touching the filesystem).
func TestDeleteAgentModeRequiresYes(t *testing.T) {
	...
	root.AddCommand(NewCmdTemplate(f, ioStreams))
	root.SetArgs([]string{"template", "delete", "vm/gpu-training"})
...
func TestDeleteHasYesFlag(t *testing.T) {
	...
	tmplCmd := NewCmdTemplate(f, ioStreams)

	var deleteCmd *cobra.Command
	for _, sub := range tmplCmd.Commands() {
		if sub.Name() == "delete" {
			deleteCmd = sub
			break
		}
